...
function drop_missing_warning(wInstance, color) {
	...
	if (wInstance.settings.container.find('.missing-data-warning').css('color')==color && wInstance.settings.container.find('.missing-data-warning').is(':animated')) return;
...
function stationBasedDataFetchAjax ( evt ) {
	...
	wInstance.currAjax = $.ajax( {
		...
		success : function( servermsg ) {
			...
			var dataPoints = {
				date_recorded : { type : 'datetime' , label : 'Day of the year' , labelShort : 'Day', tooltip: 'Day',	format : function ( dateObj , dateFormat ) { if ( !dateFormat ) { dateFormat = 'yy M d'; }; return $.datepicker.formatDate( dateFormat , dateObj ); } , highchart : { axis : { dateTimeLabelFormats : { second:'%b %e' , minute:'%b %e' , hour:'%b %e' , day:'%b %e \'%y' , week:'%b %e \'%y' , month:'%b %e \'%y' , year:'%b %e %Y' } } } },
				temp : { type : 'float' , label : 'Average temperature' , labelShort : 'Average temperature (°F)' , tooltip:'Avg temp',  range: [0,100] , format : function ( val ) { return Math.round( val ) + '°F'; } }
			};
...
function calculatedSolarDataFetch( evt ) {
	...
	for (mid in this.markers) {
		...
		var dataPoints = {
			date : { type : 'datetime' , label : 'Day of the year' , labelShort : 'Day' , tooltip: 'Day', format : function (dateObj, dateFormat) { if (!dateFormat) { dateFormat = 'yy M d'; }; return $.datepicker.formatDate(dateFormat, dateObj); } , highchart : { axis : { dateTimeLabelFormats : { second : '%b %e', minute: '%b %e', hour : '%b %e', day : '%b %e \'%y', week : '%b %e \'%y', month : '%b %e \'%y', year : '%b %e %Y' } } } },
			...
			sunAngle : { type : 'float' , label : 'Maximum height of sun in sky' , labelShort : 'Max height of sun (deg)', tooltip:'Max height sun', range : [0,90] , format : function ( val ) { return Math.round( val ) + '°' ; } },
			...
			sunEnergyT : { type : 'float' , label : 'Daily theoretical energy from sun<br>(W-h/m<sup>2</sup>/d)' , labelShort : 'Sun energy (theoretical)' , tooltip: 'Theo energy', range : [0,10000] , format : function ( val ) { return Math.round( val ) + ' Watt-hours per meter<sup>2</sup> per day'; } },
...
function fetchStatsAjax ( evt ) {
	...
	wInstance.currAjax = $.ajax( {
		...
		success : function( servermsg , textStatus , jqXHR ) {
			...
			var dataPoints = {
				date : { type : 'datetime' , label : 'Day of the year' , labelShort : 'Day', tooltip: 'Day', format : function (dateObj, dateFormat) { if (!dateFormat) { dateFormat = 'M dd'; }; return $.datepicker.formatDate(dateFormat, dateObj); } , highchart : { axis : { dateTimeLabelFormats : { second : '%b %e', minute: '%b %e', hour : '%b %e', day : '%b %e', week : '%b %e', month : '%b %e', year : '%b %e' } } } },
				// FIXME: modify lat/lng format to use E/W, N/S instead of +/- (will require a formatter function)
				...
				tempavg : { type : 'float' , label : 'Average temperature' , labelShort : 'Average temperature (°F)' , tooltip: 'Avg temp', range: [0,100] , format : function ( val ) { return Math.round( val )  + '°F'; }, info: "The temperature that you see reported for a given day is the average of the temperature measurements that were taken at that reporting station for that day, every year for thirty years. Some stations collect air temperature data every hour, and some collect air temperature data less often. But however many measurements that were taken, the number you see is the average of them for that date." },
				...
				sunAngle : { type : 'float' , label : 'Maximum height of sun in sky' , labelShort : 'Max height of sun (deg)', tooltip: 'Max height sun', range : [0,90] , format : function ( val ) { return Math.round( val ) + '°'; }, info: '<img height="75" width="75" style="float:right; vertical-align:top;" src="'+aaasClimateViz.settings.__libraryURI + '/widgets/media/45-degrees-sun.png"/>The maximum height of the sun in the sky is the maximum angle the sun reaches with respect to a person on earth and the eastern horizon.' },
				...
				sunEnergy : { type : 'float' , label : 'Daily recorded energy from sun<br>(W-h/m<sup>2</sup>/d)' , labelShort : 'Sun energy (recorded)' , tooltip: 'Rec energy', range : [0,10000] , format : function ( val ) { return ( isNaN( val ) ? '<i>No record</i>' : Math.round(val)  ); }, info: 'Watt-hours per meter squared per day is the total amount of solar energy (watts) that strikes the earth\'s surface over the course of the day on that date at that location. It is measured using real solar panels, so it may differ from the theoretical number because of cloud cover.'  },
				sunEnergyT : { type : 'float' , label : 'Daily theoretical energy from sun<br>(W-h/m<sup>2</sup>/d)' , labelShort : 'Sun energy (theoretical)' , tooltip: 'Theo energy', range : [0,10000] , format : function ( val ) { return Math.round( val ); }, info: 'Watt-hours per meter squared per day is the maximum amount of solar energy (watts) that could strike the earth\'s surface over the course of the day on that date at that location. This amount is calculated from the angle that the sun\'s rays hit that location at that time of year.' },
				sunImage : { type : 'string' , label : 'How the sun appears at its highest point' , labelShort : 'Sun Appearance' , format : function ( val ) { return '<img src="' + val + '" />'; } }
			};
			if (aaasClimateViz.widgets[widgetIndex].settings.data.fields && aaasClimateViz.widgets[widgetIndex].settings.data.fields.length > 0) {
				var dpInclude = [];
