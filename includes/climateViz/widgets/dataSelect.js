	...
	wInstance.currAjax = $.ajax( {
		...
		success : function( servermsg , textStatus , jqXHR ) {
			...
			var dataPoints = {
				...
				sunEnergy : { type : 'float' , label : 'Daily recorded energy from sun<br>(W-h/m<sup>2</sup>/d)' , labelShort : 'Rec energy (W-h/m<sup>2</sup>/d)' , tooltip: 'Rec energy', range : [0,10000] , format : function ( val ) { return ( isNaN( val ) ? '<i>No record</i>' : Math.round(val)  ); }, info: 'Watt-hours per meter squared per day is the total amount of solar energy (watts) that struck the earth\'s surface over the course of the day on that date at that location. It is measured using real solar panels, so it may differ from the theoretical number because of cloud cover.'  },
